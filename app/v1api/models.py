...
class CustomerOrders:
    ...
    def make_order(self, username, phone_number, myitems=list()):
        ...
        if not phone_number.isdigit() or phone_number.strip() == "" or len(phone_number) < 10 or len(phone_number) > 12:
            return jsonify({'error': 'wrong phone number format'}, 400)
    ...
    def get_an_order(self, order_id):
        ...
        return jsonify({'error':'order doesnot exit'}, 400)
    ...
    def update_status(self, order_id, status):
        ...
        for order in self.orders:
            if order['order_id'] == order_id:
                order['status'] = status
                return jsonify({'status updated':self.orders}, 200)
        return jsonify({'error':'The order id doesnot exist'}, 400)
